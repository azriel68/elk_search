...
class ListELK
{
     ...
     */
    function __construct($id, $page = 0, $sortfield='', sortorder='')
    {
        ...
        $this->param->hiddenColumns=array();
        $this->param->links=array();
        $this->param->types=array();
        $this->param->allowFieldsSelect=false;
        $this->param->searchFields=array();
        $this->param->list = new \stdClass();
        $this->param->list->messageEmpty = $langs->trans('ListMessageNothingToShow');
        $this->param->list->title = $langs->trans('List');
        $this->param->list->headerSerch='';
        $this->param->list->massActions=array();
        $this->param->list->limit=new stdClass();
        $this->param->list->limit->page = $page;
        $this->param->list->limit->nbLines = $conf->liste_limit;
        $this->param->list->sortfield = $sortfield;
        $this->param->list->sortorder = $sortorder;

        include_once DOL_DOCUMENT_ROOT.'/core/class/html.form.class.php';
        $this->form = new Form($this->db);
    }

    function setSearchField($field, $type = true, $allow_is_null = false, $table='') {
        $this->param->searchFields[$field]=new stdClass();
        $this->param->searchFields[$field]->field = $field;
        $this->param->searchFields[$field]->type = $type;
        $this->param->searchFields[$field]->allow_is_null = $allow_is_null;
        $this->param->searchFields[$field]->table = $table;
    }
    ...
    {
        return !empty($this->param->searchFields[$key]->allow_is_null);
    }
    /**
     * @param string    $key    field name
     * @return array
     */
    private function getSearchKey($key)
    {
        $TPrefixe = array();
        if(!empty($this->param->searchFields[$key]->table))
        {
            if (!is_array($this->param->searchFields)) $this->param->searchFields = array($this->param->searchFields);

            foreach ($this->param->searchFields as $prefix_table)
            {
        ...
        $TKey=array();
        if(!empty($this->param->searchFields[$key]))
        {
            if (!is_array($this->param->searchFields[$key]->field)) $this->param->searchFields[$key]->field = array($this->param->searchFields[$key]->field);

            foreach ($this->param->searchFields[$key]->field as $i => $field)
            {
     ...
     * @return string
     */
    public function render($index, $queryParam)
    {
        global $conf;
...
//		$search_button.= '&nbsp;<a href="#" onclick="Listview_clearSearch(this);" class="list-reset-link">'.img_searchclear().'</a>';
    ...
    */
    ...
    private function initHeader(&$TParam)
    {
        ...
        {
            $selectedfields = GETPOST('ListELK' .$this->id.'_selectedfields');
            ...
            $selectedfields = $this->form->multiSelectArrayWithCheckbox('ListELK' .$this->id.'_selectedfields', $TFieldVisibility, $contextpage);	// This also change content of $arrayfields_0
        }
        ...
        uasort($THeader,array('ListELK','sortHeaderRank'));
    ...
    {
        if(!empty($this->param->list->limit->nbLine)) {
            $this->queryParam['from'] = $this->param->list->page * $this->param->list->limit->nbLine;
            $this->queryParam['size'] = $this->param->list->limit->nbLine + 1;
        }
    ...
    private function run(&$THeader, &$TField, $sql)
    {
        ...
        $this->TTotalTmp=array();
        $this->THideFlip = array_flip($this->param->hiddenFields);
